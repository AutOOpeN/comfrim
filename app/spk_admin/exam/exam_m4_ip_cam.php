...
<body>
    ...
    <div class="container">
        <div class="h3 text-center">
            สร้างผู้มีสิทธิเข้าสอบ ชั้นมัธยมศึกษาปีที่ 4 โครงการ IP CAMBRIDGE
        </div>
        <div class="pt-4 pb-4">
        ...
        <form action="" method="post" class="form">
            ...
            <button type="submit" class="btn btn-primary"
                onClick="return confirm('ยืนยันการ สร้างผู้มีสิทธิเข้าสอบ ชั้นมัธยมศึกษาปีที่ 4 โครงการ IP CAMBRIDGE  ?')">สร้างผู้เข้าสอบ</button>

        </form>
        <hr class="p4">
        <a class="" href="../report/m4_ip.php" role='btn'>รายชื่อผู้มีสิทธิสอบ ชั้นมัธยมศึกษาปีที่ 4 โครงการ IP
            CAMBRIDGE </a>
        <?php
        $conn = new PDO("mysql:host=$servername;dbname=$dbname;charset=utf8", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $last_create_ex = $conn->prepare("SELECT * FROM log_crate_exam WHERE plan_name  LIKE 'm4_ip_cam' ORDER BY last_create DESC LIMIT 1 ");
        ...
        if (!empty($_POST['room_max'])) {
            ...
            try {
                ...
                $sql = "SELECT * FROM m4_ip WHERE status = 4 AND m1s290 LIKE '4'  ORDER BY m1s001 ASC";
                $data = $conn->prepare($sql);
            $data->execute();
                $conn->exec("TRUNCATE TABLE ex_m4_ip_cam");
                $conn->exec("UPDATE room_m4_ip_cam SET r_current = 0");
                ...
                while ($row_data = $data->fetch()) {
                    ...
                    $m1_smte_exec = $conn->prepare("SELECT * FROM room_m4_ip_cam WHERE  r_current <= $r_max LIMIT 1");
                    ...
                    $str_sql_update = "UPDATE room_m4_ip_cam SET r_current = " . $tmp_current . " WHERE rid = " . $tmp_rid;
                ...
                $conn->exec("INSERT INTO log_crate_exam(plan_name,last_create) VALUES ('m4_ip_cam', '$log_time')");
                $last_create_ex->execute();
