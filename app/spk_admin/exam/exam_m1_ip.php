...
<body>
    ...
    <div class="container">
        <div class="h3 text-center">
            สร้างผู้มีสิทธิเข้าสอบ ชั้นมัธยมศึกษาปีที่ 1 โครงการ IP 
        </div>
        <div class="pt-4 pb-4">
        ...
        <form action="" method="post" class="form">
            ...
            <button type="submit" class="btn btn-primary" onClick="return confirm('ยืนยันการ สร้างผู้มีสิทธิเข้าสอบ ชั้นมัธยมศึกษาปีที่ 1 โครงการ IP  ?')">สร้างผู้เข้าสอบ</button>
            
        </form>
            <hr class="p4">    
        <a class="" href="../report/m1_ip.php" role='btn'>รายชื่อผู้มีสิทธิสอบ ชั้นมัธยมศึกษาปีที่ 1 โครงการ IP </a>
        <?php
        $conn = new PDO("mysql:host=$servername;dbname=$dbname;charset=utf8", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $last_create_ex = $conn->prepare("SELECT * FROM log_crate_exam WHERE plan_name  LIKE 'm1_ip' ORDER BY last_create DESC LIMIT 1 ");
        ...
        if (!empty($_POST['room_max'])) {
            ...
            try {
                ...
                $sql = "SELECT * FROM m1_ip WHERE status = 4 AND m1s290 LIKE '2'  ORDER BY m1s001 ASC";
                $data = $conn->prepare($sql);
                $data->execute();
                $conn->exec("TRUNCATE TABLE ex_m1_ip");
                $conn->exec("UPDATE room_m1_ip SET r_current = 0");
                ...
                while ($row_data = $data->fetch()) {
                    ...
                    $m1_smte_exec = $conn->prepare("SELECT * FROM room_m1_ip WHERE  r_current <= $r_max LIMIT 1");
                    $sql_insert = "INSERT INTO ex_m1_ip (ref1,ref2,ex_room) VALUES (?,?,?)";
                    ...
                    $str_sql_update = "UPDATE room_m1_ip SET r_current = " . $tmp_current . " WHERE rid = " . $tmp_rid;
                ...
                $conn->exec("INSERT INTO log_crate_exam(plan_name,last_create) VALUES ('m1_ip', '$log_time')");
                $last_create_ex->execute();
