    ...
    <div class="container">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <fieldset class="myfieldset">
                <legend class="myfieldset">สำเนาหนังสือรับรองกำลังศึกษาอยู่ หรือ สำเนา ปพ.1 หน้าที่ 1 </legend>
                <label class="form-label" for="file2">เลือกสำเนาหนังสือรับรองกำลังศึกษาอยู่ หรือ สำเนา ปพ.1 หน้าที่ 1</label>
        ...
        if (isset($_FILES['file2'])) {
            $file01_extension = pathinfo(basename($_FILES['file2']['name']), PATHINFO_EXTENSION);
            if ($file01_extension != "") {
                $new_file01_name = $_POST['tbl'] . '_img_' . $_POST['student_id'] . "." . $file01_extension;
                $file01_path     = "/var/www/html/app/file2/";
            ...
            if ($file01_extension != "") {
                $status_file3 = move_uploaded_file($_FILES['file2']['tmp_name'], $upload_path01);
                //echo '123';
                if ($status_file3 == false) {
                    echo "ไม่สามารถส่งไฟล์ สำเนาหนังสือรับรองกำลังศึกษาอยู่ หรือ สำเนา ปพ.1 หน้าที่ 1";
                    //exit();
                }
                try {
                    $conn = new PDO("mysql:host=$servername;dbname=$dbname;charset=utf8", $username, $password);
                    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                    $sql_update = "UPDATE " . $_POST['tbl'] . " SET file_name_2 =  '" . $new_file01_name . "' WHERE  m1s003 = '" . $_POST['student_id'] . "'";
                    // echo $sql_update;
