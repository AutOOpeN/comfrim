...
switch ($sub_plan) {
    case '2':
        $str_sub_plan = "IP แผนการเรียน ภาษาอังกฤษ - คณิตศาสตร์";
        break;
    case '3':
        $str_sub_plan = "IP แผนการเรียน วิทยาศาสตร์ - คณิตศาสตร์";
        break;
    case '4':
        $str_sub_plan = "IPC แผนการเรียน Cambridge";
    break;
}
$exam_id="";
$exam_room="";
...
$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    $str_sql_ip = "SELECT * FROM ex_m4_ip WHERE ref1 = '" . $ref1 . "'";
    $chekc_ip = $conn->query($str_sql_ip);
    if ($chekc_ip == 1) {
        $str_sql = "SELECT * FROM ex_m4_ip WHERE ref1 = '" . $ref1 . "'";
    ...
    $str_sql_ip_cam = "SELECT * FROM ex_m4_ip_cam WHERE ref1 = '" . $ref1 . "'";
    $chekc_cambridge = $conn->query($str_sql_ip_cam);
    if ($chekc_cambridge == 1) {
        $str_sql = "SELECT * FROM ex_m4_ip_cam WHERE ref1 = '" . $ref1 . "'";
        $stmt = $conn->prepare($str_sql);
        $stmt->execute();
        while ($a = $stmt->fetch()) {
            $exam_id = $a['ex_id'];
...
<table>
    <tr>
        ...
        <td width = "500" colspan="2" style="text-align: right;"><h2>เลขที่นั่งสอบ: <u> $exam_id </u><br /> ห้องสอบ: <u> $exam_room </u></h2></td>
    </tr>
    <tr>
    ...
    <tr>
        <td width = "500" colspan="2"  style="text-align: center;"><h3>ชั้นมัธยมศึกษาปีที่ 4 $strEducationYear</h3></td>
    </tr>
</table>
    ...
    <tbody>
        <tr>
            ...
            <td width="450" >
                ...
                <p>โครงการที่สมัคร : <u> $str_sub_plan </u></p>
...
$str_comment = <<<EOD
(นำส่วนนี้มาแสดงในวันสอบ และบัตรประจำตัวประชาชน ถ้าไม่มีจะไม่อนุญาตให้เข้าสอบ)* <u>$strDayExamM4</u> กรุณานำ <u>ดินสอดำ 2B</u> มาใช้ในการสอบ
EOD;
$str_comment2 = <<<EOD
(นำส่วนนี้มาแสดงในวันสอบ ถ้าไม่มีจะไม่อนุญาตให้เข้าสอบ)* <u>$strDayExamM4</u> กรุณานำ <u>ดินสอดำ 2B</u> มาใช้ในการสอบ
EOD;
