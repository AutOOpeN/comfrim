        ...
        <?php
          if (isset($_POST['id_card_print'])) {
              try {
                ...
                $m1_s_Result  = $conn->query("SELECT count(*) FROM m1_ip WHERE  m1s003 = '" . $_POST['id_card_print'] . "'" ." AND status = 4")->fetchColumn();
                if ($m1_s_Result == 1) {
                    $tmp_sql = "SELECT * FROM m1_ip WHERE m1s003 = '" . $_POST['id_card_print'] . "' AND status = 4";
                    ...
                    switch ($student_plan) {
                        case '2':
                            $str_sub_plan = "โครงการ IP (International Program)";
                            break;
