...
try {
    ...
    $sql = "SELECT * FROM m1_ip WHERE status = 4 ORDER BY m1s001 ASC";
    ...
    while ($row_data = $data->fetch()) {
        if ($row_data['m1s290'] == '4'){
            $m1_smte_exec = $conn->prepare("SELECT * FROM room WHERE rid = 30 AND r_current <= 39 LIMIT 1");
            $sql_insert = "INSERT INTO ex_m1_ip_cam (ref1,ref2,ex_room) VALUES (?,?,?)";
        }else{
            $m1_smte_exec = $conn->prepare("SELECT * FROM room WHERE rid > 20 AND r_current <= 39 LIMIT 1");
